...
Game.Event.OnKeyPressed:Connect(function ()
    if Gamepad.isDown(Gamepad.KeyCodes.DPADDOWN) then
        ...
        if handSlot ~= nil then -- handSlot will be nil when player isn't loaded
            ...
            Debug.message("Player is holding item: "..itemName)
        end
    end
end)
...
Game.Event.OnKeyPressed:Connect(function ()
    if Gamepad.isDown(Gamepad.KeyCodes.DPADDOWN) then
        ...
        if slot ~= nil then -- slot will be nil when player isn't loaded
            ...
            Debug.message("Player has item in slot 1: "..itemName)
        end
    end
end)
...
Game.Event.OnKeyPressed:Connect(function ()
    if Gamepad.isDown(Gamepad.KeyCodes.DPADDOWN) then
        ...
        if slot ~= nil then -- slot will be nil when player isn't loaded
            ...
            Debug.message("Player has in slot 1 "..itemCount.." items")
        end
    end
end)
...
Game.Event.OnKeyPressed:Connect(function ()
    if Gamepad.isDown(Gamepad.KeyCodes.DPADDOWN) then
        ...
        if slot ~= nil then -- slot will be nil when player isn't loaded
            ...
            Debug.message("Player has in slot 1 "..itemData.." item variation")
        end
    end
end)